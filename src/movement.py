...
def callback(data): 
    ...
    x = float(raw.split('&')[0])
    y = float(raw.split('&')[1])
    width = float(raw.split('&')[2])
    height = float(raw.split('&')[3])
    co_ords = (x,y)
...
def listener():
    rospy.init_node('movement_listen', anonymous=True)
    rospy.Subscriber("/co-ords", String, callback)
    rospy.Subscriber("/robot/range/left_hand_range/state", Range, callback2)
...
def map_keyboard():
    ...
    #   key: (function, args, description)
        '9': (set_j, [left, lj[0], 0.05], "left_s0 increase"),
        '6': (set_j, [left, lj[0], -0.05], "left_s0 decrease"),
        '8': (set_j, [left, lj[1], 0.05], "left_s1 increase"),
        '7': (set_j, [left, lj[1], -0.05], "left_s1 decrease"),
        'o': (set_j, [left, lj[2], 0.05], "left_e0 increase"),
        'y': (set_j, [left, lj[2], -0.05], "left_e0 decrease"),
        'i': (set_j, [left, lj[3], 0.05], "left_e1 increase"),
        'u': (set_j, [left, lj[3], -0.05], "left_e1 decrease"),
        'l': (set_j, [left, lj[4], 0.05], "left_w0 increase"),
        'h': (set_j, [left, lj[4], -0.05], "left_w0 decrease"),
        'k': (set_j, [left, lj[5], 0.05], "left_w1 increase"),
        'j': (set_j, [left, lj[5], -0.05], "left_w1 decrease"),
        '.': (set_j, [left, lj[6], 0.05], "left_w2 increase"),
        'n': (set_j, [left, lj[6], -0.05], "left_w2 decrease"),
